...
# 依工作流比對缺失 → Manager 設 weak → 補 4 個指定節點 → 裝 5 個模型 → 修 insightface v0.7/antelopev2
set -euo pipefail
...
VENV="/venv"
COMFY_PORT="8188"
BASE=""   # 例：--base https://你的Cloudflare域名
...
if [[ "$SKIP_WAIT" = "0" ]]; then
  ...
  until curl -fsS "${BASE}/object_info" >/dev/null 2>&1; do
    sleep 3; t=$((t+3))
    [[ $t -ge $MAX_WAIT ]] && { warn "逾時，繼續執行"; break; }
  done
else
  log "跳過等待 ComfyUI。"
fi
...
# ===== 收尾：再比一次（提醒 Reload）=====
log "安裝完成。重新比對（若仍有缺，請在 UI 做：Manager → Reload Custom Nodes）..."
python3 - "$BASE" "$WF" <<'PY'
import json, sys, urllib.request
base, wf_path = sys.argv[1], sys.argv[2]
need=set()
